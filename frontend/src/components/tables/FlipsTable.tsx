...
import type { FlipResult } from "@/lib/types/api";
...
export function FlipsTable({ data }: Props) {
  if (!data.length) {
    return (
      <Card>
        ...
        <CardContent>
          <p className="text-sm text-muted-foreground">No flips found with current filters.</p>
        </CardContent>
      </Card>
    );
  }
    ...
    <Card>
      <CardHeader className="pb-2">
        <CardTitle className="text-sm font-medium">
          Flip Detection — {data.length} candidate{data.length !== 1 ? "s" : ""}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            <thead className="bg-muted/50 border-b">
              <tr>
                {["Address", "Buy Date", "Sell Date", "Hold", "Buy $", "Sell $", "Profit", "Buyer"].map((h) => (
              ...
              {data.map((flip, i) => (
                <tr key={i} className="border-b hover:bg-muted/30 transition-colors">
                  <td className="px-3 py-1.5">
                    <Link href={`/dashboard/parcels/${flip.parcel_id}`} className="text-blue-600 hover:underline max-w-[160px] block truncate">
                  ...
                  <td className="px-3 py-1.5">
                    <Badge variant="default" className="text-xs">+{(flip.profit_pct * 100).toFixed(0)}%</Badge>
                  </td>
